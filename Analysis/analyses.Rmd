---
title: "Analyses for SII Experiment 4"
output: 
  html_notebook:
  code_folding: hide
---
...
check_desc <- check %>%
  ...
  rowwise() %>%
  mutate(across(c(mean, sd), ~ forma(.x)))
...
rt_2f_outlier <- rt_2f %>%
  ...
  geom_boxplot() +
  labs(x = "Reason type", y = "Mean response latency in log(ms)",
      fill = "Probe type") +
  scale_color_manual(values = c("#849AB9", "#465263")) +
  geom_point(
...
For interpretative ease, the participant means have been converted back to milliseconds before computing descriptive statistics.

#### Lineplot
```{r rt 2f lineplot, message = FALSE}
rt_2f_lineplot <- rt_2f_desc %>%
  ggplot(., aes(reason_type, mean, color = probe_type, group = probe_type)) +
  labs(x = "Reason type", y = "Mean response latency in ms",
    color = "Probe type") +
  scale_color_manual(values = c("#849AB9", "#465263")) +
  geom_line() +
  geom_point(size = 3) +
...
er_2f_outlier <- filter(er_2f, is_extreme == FALSE) %>%
...
label_rate <- d_long %>%
  ...
  group_by(label) %>%
  summarize(.groups = "drop",
    across(c(identity, necessity, baserate), ~ mean(.x, na.rm = TRUE)))
label_suff <- d_long %>%
  filter(probe_type == "im") %>%
  select(label, reason_type, sufficiency = rating_sufficiency) %>%
  group_by(label, reason_type) %>%
  summarize(.groups = "drop", across(sufficiency, ~ mean(.x, na.rm = TRUE)))
label <- d_long %>%
...
by_label_sii_identity <- filter(label, reason_type == "suff") %>%
  ggplot(., aes(x = identity, y = sii_effect)) +
  labs(x = "Mean identity rating", y = "SII effect") +
  geom_smooth(method = "lm", formula = y ~ x, color = "#465263",
    linewidth = 0.5, fill = "#849AB9") +
  geom_abline(intercept = 0, slope = 0, color = "#465263") +
  geom_point(color = "#465263") +
  geom_label_repel(label = filter(label, reason_type == "suff")$label,
    size = 4.3, box.padding = 0.2, family = "Nunito", color = "#465263") +
  theme_cs_talk()
by_label_sii_identity
```

#### Necessity
```{r by-label sii necessity, message = FALSE}
by_label_sii_necessity <- filter(label, reason_type == "suff") %>%
  ...
  geom_label_repel(label = filter(label, reason_type == "suff")$label,
    size = 4.3, box.padding = 0.2, family = "Nunito", color = "#465263") +
  theme_cs_talk()
by_label_sii_necessity
```

#### Baserate
```{r by-label sii baserate, message = FALSE}
by_label_sii_baserate <- filter(label, reason_type == "suff") %>%
  ...
  geom_label_repel(label = filter(label, reason_type == "suff")$label,
    size = 4.3, box.padding = 0.2, family = "Nunito", color = "#465263") +
  theme_cs_talk()
by_label_sii_baserate
```

#### Sufficiency
```{r by-label sii sufficiency, message = FALSE}
by_label_sii_sufficiency <- filter(label, reason_type == "suff") %>%
...
for (id in unique(d_long$item_id)) {
  sub <- d_long %>%
    filter(item_id == id) %>%
    select(item_id, subject_id, label, probe_type, reason_type, rt_M2SD_log)

  sub_wide <- sub %>%
    select(-label) %>%
    pivot_wider(names_from = probe_type, values_from = c(rt_M2SD_log)) %>%
    mutate(.keep = "unused", inference_effect = im - io)

  # Descriptives
  sub_desc <- sub %>%
    group_by(item_id, probe_type, reason_type, label) %>%
    get_summary_stats(rt_M2SD_log, type = "mean") %>%
    select(-n, -variable) %>%
    pivot_wider(names_from = probe_type, values_from = c(mean, label)) %>%
    mutate(.keep = "unused",
      inference_effect = mean_im - mean_io) %>%
    pivot_wider(names_from = reason_type,
      values_from = inference_effect, names_prefix = "inference_")

  # Two-sided Welch two-sample t-test
  sub_t <- sub_wide %>%
    t_test(
      inference_effect ~ reason_type,
      paired = FALSE,
  ...
  # Cohens dz
  sub_d <- sub_wide %>%
    cohens_d(inference_effect ~ reason_type,
      paired = FALSE, ref.group = "ctrl") %>%
    select(`cohen's d` = effsize)

  # Merge
  sub_all <- cbind(sub_desc, sub_t, sub_d)
...
item_results <- item_results %>%
  rowwise() %>%
  mutate(across(
    c(inference_ctrl, inference_suff, t, df, `cohen's d`), ~ forma(.x))) %>%
  mutate(p = formp(p))
```
...
by_label_disc_necessity <- label_disc %>%
  ...
  geom_label_repel(size = 4.3, label = label_disc$label, box.padding = 0.2,
    family = "Nunito", color = "#465263") +
  theme_cs_talk()
by_label_disc_necessity
```

#### Baserate
```{r by-label discounting baserate, message = FALSE}
by_label_disc_baserate <- label_disc %>%
  ...
  geom_label_repel(size = 4.3, label = label_disc$label, box.padding = 0.2,
    family = "Nunito", color = "#465263") +
  theme_cs_talk()
by_label_disc_baserate
```

#### Sufficiency
```{r by-label discounting sufficiency, message = FALSE}
by_item_disc_sufficiency <- label_disc %>%
...
ggexport(check_plot, width = 7, height = 6,
  filename = here::here("Analysis/check.pdf"))
ggexport(rt_2f_outlier, width = 6, height = 5,
  filename = here("Analysis/rt-2f-outlier.pdf"))
ggexport(rt_2f_qq, width = 7, height = 7,
  filename = here("Analysis/rt-2f-qq.pdf"))
ggexport(rt_2f_lineplot, width = 6, height = 5,
  filename = here::here("Analysis/rt-2f-lineplot.pdf"))
ggexport(er_2f_outlier, width = 5, height = 5,
  filename = here::here("Analysis/er-2f-outlier.pdf"))
ggexport(er_2f_qq, width = 7, height = 7,
  filename = here("Analysis/er-2f-qq.pdf"))
ggexport(er_2f_lineplot, width = 6, height = 5,
  filename = here::here("Analysis/er-2f-lineplot.pdf"))
ggexport(rt_3f_outlier, width = 8, height = 5,
  filename = here("Analysis/rt-3f-outlier.pdf"))
ggexport(rt_3f_qq, width = 7, height = 7,
  filename = here("Analysis/rt-3f-qq.pdf"))
ggexport(rt_3f_lineplot, width = 8, height = 5,
  filename = here::here("Analysis/rt-3f-lineplot.pdf"))
ggexport(by_label_sii_identity, width = 7, height = 6,
  filename = here::here("Analysis/by-label-sii-identity.pdf"))
ggexport(by_label_sii_necessity, width = 7, height = 6,
  filename = here::here("Analysis/by-label-sii-necessity.pdf"))
ggexport(by_label_sii_baserate, width = 7, height = 6,
  filename = here::here("Analysis/by-label-sii-baserate.pdf"))
ggexport(by_label_sii_sufficiency, width = 7, height = 6,
  filename = here::here("Analysis/by-label-sii-sufficiency.pdf"))
ggexport(by_label_disc_identity, width = 7, height = 6,
  filename = here::here("Analysis/by-label-disc-identity.pdf"))
ggexport(by_label_disc_necessity, width = 7, height = 6,
  filename = here::here("Analysis/by-label-disc-necessity.pdf"))
ggexport(by_item_disc_baserate, width = 7, height = 6,
  filename = here::here("Analysis/by-label-disc-baserate.pdf"))
ggexport(by_item_disc_sufficiency, width = 7, height = 6,
