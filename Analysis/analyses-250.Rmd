...
manipulation_check <- d_long %>%
    filter(probe_type == "im") %>%
    select(subject_id, reason_type, sufficiency = rating_sufficiency) %>%
    group_by(subject_id, reason_type) %>%
    summarize(mn_sufficiency = mean(sufficiency, na.rm = TRUE)) %>%
    pivot_wider(names_from = "reason_type", values_from = "mn_sufficiency")

# Descriptives
m_suff <- format(round(mean(manipulation_check$suff), 2), nsmall = 2)
m_ctrl <- format(round(mean(manipulation_check$ctrl), 2), nsmall = 2)
sd_suff <- format(round(sd(manipulation_check$suff), 2), nsmall = 2)
sd_ctrl <- format(round(sd(manipulation_check$ctrl), 2), nsmall = 2)
# One-sided paired t-test
t1 <- t.test(manipulation_check$suff, manipulation_check$ctrl,
...
d_z <- format(round(mean(manipulation_check$suff - manipulation_check$ctrl) /
                    sd(manipulation_check$suff - manipulation_check$ctrl), 2),
              nsmall = 2)
```
We included a manipulation check for reason type. The sufficient reasons are supposed to sufficiently explain the behaviors, whereas the control reasons are supposed to insufficiently explain the behavior (or not explain the behavior at all). We thus asked participants at the end of the study to rate the sufficiency of each reason for the respective behavior and expected the sufficient reasons to score higher in their sufficiency ratings than the control reasons. To test this, we conducted a paired t-test. On average, participants gave significantly higher sufficiency ratings in the sufficient condition (M = `r m_suff`, SD = `r sd_suff`) than in the control condition (M = `r m_ctrl`, SD = `r sd_ctrl`), t(`r t1$parameter`) = `r round(t1$statistic, 2)`, `r t1_p`, d~z~ = `r d_z`. We infer from this, that our manipulation of reason type worked.
